...
@pydef mutable struct Guiapp <: tk.Tk
    __init__(self) = begin
        ...
        mainlab.place(relx = 0.43, rely = 0.63)
        ...
        function move_obj(xmove, xmove1, ymove, ymove1, anglestart, distin)
            ...
            function calcre()
                canres.after_cancel(aftervar)
                anidraw()
            end
            aftervar = canres.after(40, move_obj, xmove, xmove1, ymove, ymove1, anglestart, distin)
        ...
        function anidraw()
            ...
            anglestart = ainc
            timeo = sqrt(4*(mathmod.pi^2)*((distin*10^23)^3)/(gconst*m1in))
            timect = timeinc
            xmove = distin
            ymove = 0
            xmove1 = distin*cos(anglestart)
